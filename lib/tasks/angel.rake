...
task :testing => [:environment] do
  ...
  companies.each do |company|
    ...
    if company.angel_id
      ...
      if logo_url && logo_url.to_s.length > 1
        if logo_url.to_s.index('g?buster')

          p.angel_logo_url = logo_url.to_s
          p.angel_thumb_url = thumb_url.to_s
          str = logo_url.to_s
          logo = str.slice(0..(str.index('g?buster')))
          if  File.extname(logo)

            extname = File.extname(logo)
            basename = File.basename(logo, extname)
            file = Tempfile.new([basename, extname])
            file.binmode
            if open(URI.parse(logo)) 
            open(URI.parse(logo)) do |data|
