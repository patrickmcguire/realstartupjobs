...
companies.each do |company|
  ...
  elsif  company.resumator_feed.to_s.length == 0 && company.jobscore_feed.to_s.length == 0 && company.name.length >=1 && Nokogiri::XML(open("http://www.simplyhired.com/a/job-feed/rss/l-10003/fcn-"+company_subbed+"/frl-newgrad/fsr-primary/fem-employer")).xpath("/rss/channel/item").count > 0
  ...
  else #worst case fallback to indeed
    company_name = company.name.to_s
    if company_name
